...
export default function RoamSplitScreen({ trip, userId, setActiveTab, showToast }) {
  ...
  if (!tripId) {
    ...
    async function handleDelete(group) {
      const res = await deleteSplitGroup(group, userId);
      if (!res.ok) { showToast(res.error, "error"); return; }
      setSplitGroups((prev) => prev.filter((g) => g.id !== group.id));
        ...
        <>
          <div className="rs-balance-hero">
            ...
            <div className="rs-hero-foot">
              ...
              <br/><span style={{ opacity: 0.8, fontSize: "0.95em" }}>Created by {travellers.find(t => t.id === selectedTrip?.creatorId)?.name || travellers?.[0]?.name || "Someone"}</span>
            </div>
          </div>
